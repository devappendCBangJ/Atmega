...
● Basic
		...
		4) 구성
			...
			(2) 임베디드 소프트웨어
				...
				1] 시스템 소프트웨어(System Software)
					[1] 정의 : 시스템의 하드웨어 관리, 응용프로그램에게 효율적이고 안전한 interface 제공 ♣♣♣
					[2] 종류 ♣♣♣
						1]] 운영체제(OS) ♣♣♣
							[[1]] 특징 ♣♣♣
								1 큰 임베디드시스템 처리
	...
	2. 프로세서
		...
		(1) 마이크로 프로세서(MPU = MicroProcessor Unit)
			...
			2] 구조
				// 강의자료 참조 ♣♣♣
		(2) 마이크로 컨트롤러(MCU = MicroController Unit) = One-chip = 마이콤
			1] 특징
...
● Timer & Counter ♣♣
	...
	4) Counter Pin ♣♣♣
			...
			3] 비교출력 모드 ♣♣♣
				...
				[2] 위상정정 PWM
					COM01 COM00 : 00 = 사용 안함, 01 = 예약, 
									10 = 카운트 업 중에 TCNT0 == OCR0면 OC0핀 클리어 / 카운트 다운 중에 TCNT0 == OCR0면 OC0핀 셋, 
									11 = 카운트 업 중에 TCNT0 == OCR0면 OC0핀 셋 / 카운트 다운 중에 TCNT0 == OCR0면 OC0핀 셋(이게 맞나??? 클리어 아닌가???)
				[4] 고속 PWM
					COM01 COM00 : 00 = 사용 안함, 01 = 예약, 
		...
		(3) Interrupt 마스크 레지스터
			...
			- TOIE0 = 1 : 카운트 Overflow Interrupt 활성화
			- OCIE0 = 1 : 출력 Compare Interrupt 활성화
		(4) 전역 Interrupt 활성화
			1] 함수 호출 way : sei();
			2] 레지스터 way : SREG |= 1<<I;
		(5) Interrupt 벡터
		...
		(6) 활용
			...
			2] RC 서보모터
				[1] 작동 원리
					- DC모터 회전축 + 감속기어 -> 토크 상승
					- POT활용, 명령각도와 비교 -> 토크 암 각도 측정 -> 위치 제어
				[2] 작동 방법
					- 주기 : 10 ~ 20ms
					- OnDuty
						0.6ms : 0도
...
● Practice Basic
	...
	5. DC모터
		1) 비교
			...
			(2) RC서보
				1] 장점 : 제어 easy
				2] 단점 : DC모터, POT 수명 문제 -> BLDC, Encoder 사용
				3] 활용 : 무선조종장치, 취미로봇, 간이장치
			(3) Step모터
				1] 장점
					[1] 속도, 위치제어 easy
					[2] 긴 수명
		...
		2) 원리
			(1) DC모터
				1] 회전자 : 코일
					- 회전자의 코일에 브러쉬, 정류자에 의해 계속적으로 서로 다른 극성 전기 공급
					- 영구자석, 회전자의 코일에 의한 인력, 반발력 -> 회전
				2] 고정자 : 영구자석
			(3) Step모터
				1] 회전자 : 이가 나있는 금속휠 or 영구자석
					- 회전자 회전 : 자기력선이 가장 잘 통과하기 위해
				2] 고정자 : 여러 조의 코일
		3) 모터 드라이버
			(1) 작동원리 : H-bridge
	...
	+a)
		1) 다중파일 이용 프로그래밍
			...
			(2) 동작 순서
				1] 별도 헤더파일 or C파일 작성
				2] 라이브러리 형태, 각 기능을 사용자 정의 함수 형태로 작성
				3] 각각 C파일 따로 컴파일 + 링크 시 결합
